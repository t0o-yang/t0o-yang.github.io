...
tags: ["Vue","Proxy"]
categories: 百日博客计划
date: 2023-04-08 23:21:26
description: Proxy实现数据绑定
---

# 前言

CDN使用Vue3的方式为切入点，一点点实现数据绑定，有三点不足之处：
1. 对嵌套对象，依赖会有重复；

和name一样，more.like应该只有两个订阅者，一个是模板语法中的`{{more.like}}`，一个是input中的`v-model="more.like"`，但是发生了重复。
![dependency](../images/dependency%202023-04-08%20232544.png)

2. 目前，对一个状态修改，会造成所有订阅者做update操作

只修改了more.like，会通知name的订阅者更新。
![通知所有更新](../images/updateAll20230408233206.png)

3. 需要用 Vue.createApp 的形式绑定this，之后调用mount，与Vue3原版不同

```js
// 原版使用 createApp，与我的实现不同
const { createApp } = Vue
